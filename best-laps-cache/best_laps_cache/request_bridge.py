...
the uvicorn worker thread, and (by design now) **no best-laps payload is kept in
RAM between requests**. To serve ``GET /best-laps`` the HTTP thread therefore has
to round-trip *through* the SDF:

1. The HTTP thread calls :meth:`PendingRequests.open` to register a unique
   ``req_id`` — this creates a single-slot result holder + a :class:`threading.Event`.
...
4. The HTTP thread :meth:`PendingRequests.wait`-s on the Event (bounded timeout),
   reads the payload, builds + serialises the response, then **discards** it.

The payload lives in RAM only for the duration of one in-flight request; the slot
is deleted as soon as the waiter consumes it (or times out). Nothing best-laps-
shaped persists between requests. This is NOT a database and NOT a cache — it is a
short-lived correlation table of in-flight requests only.
...
slot dict; the :class:`threading.Event` per slot does the cross-thread wakeup.
"""
