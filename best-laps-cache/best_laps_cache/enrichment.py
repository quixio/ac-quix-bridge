"""Session + DCM enrichment of raw ticks.

``ac-telemetry-raw`` carries no ``track`` / ``carModel`` / ``driver`` /
``experiment`` / ``environment`` (``feedback_ac_raw_payload_fields``). Those
live in the session topic (track / carModel / playerName, once per session)
and in DCM (experiment / driver / environment, keyed by hostname). This
module replicates the proven ``leaderboard-service/api/live_telemetry.py``
enrichment so the live path can form the full five-key group.

The three topics use unrelated key namespaces, so raw-tick enrichment cannot
pair caches by hostname — it takes the most-recent entry of each cache
(correct for the single-sim deployment, exactly one session + one experiment
config active at a time). This mirrors ``_latest_session`` /
``_latest_experiment`` in the leaderboard.

All DCM HTTP happens on session-message arrival (rare), never per raw tick.
"""
...
class Enrichment:
    ...
    def handle_config_event(self, payload: dict[str, Any]) -> None:
        ...
        On a non-delete event we just force-refresh the experiment cache for
        the target hostname; on a delete we drop the cached entry. This keeps
        driver / experiment / environment fresh between AC sessions without
        re-implementing the contentUrl fetch.
        """
        try:
    ...
    def enrich(self, payload: dict[str, Any]) -> dict[str, str]:
        ...
        raw tick, preferring fields already on the payload, then the
        most-recent session + experiment caches.

        Any field that cannot be resolved comes back as ``""`` — the caller
        decides whether the group is complete enough to key on.
        """
        # Prefer values that the raw payload already carries (lake-enriched
        # replay payloads sometimes do); fall back to the caches.
        track = str(payload.get("track") or "").strip()
    ...
    def _latest_with_driver(
        ...
        """Most-recent experiment entry that actually carries a driver — an
        empty, constantly-touched entry must not shadow a real one."""
    ...
    def _fetch_from_dcm(self, hostname: str) -> dict[str, str]:
        """Resolve experiment / driver / environment for *hostname* via the
        DCM HTTP API (latest-version content of the matching experiment
        config). Ported from ``live_telemetry._fetch_experiment_from_dcm``.
        """
    ...
    def _fetch_latest_version_content(
        ...
        """List versions of *config_id*, pick the highest, return its content."""
        v_resp = client.get(
