...
def test_session_detail_rejects_malformed_id(client: TestClient, bad_id: str) -> None:
    r = client.get(f"/api/sessions/{bad_id}")
    assert r.status_code == 422


def test_chat_rejects_malformed_session_id(client: TestClient) -> None:
...
def test_messages_rejects_malformed_session_id(client: TestClient) -> None:
