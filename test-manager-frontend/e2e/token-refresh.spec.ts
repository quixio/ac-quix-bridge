...
test.describe("Quix Cloud Plugin Token Refresh", () => {
  ...
  test("should store token in localStorage for persistence", async ({
    page,
  }) => {
    // Wait for initial token
    await page.waitForTimeout(500);
  ...
  test("should send auth request on manual refresh trigger", async ({
    page,
  }) => {
    // Wait for initial token
    await page.waitForTimeout(500);
...
test.describe("Token Refresh Integration with API Calls", () => {
  ...
  test("should handle 403 forbidden errors with token refresh", async ({
    page,
  }) => {
    let requestCount = 0;
