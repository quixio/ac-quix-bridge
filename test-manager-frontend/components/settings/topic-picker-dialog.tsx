...
export function TopicPickerDialog({
  ...
  const fallbackTopicRef = useRef<{ topicName: string; workspaceId: string } | null>(
    isFallback && selectedTopic
      ? { topicName: selectedTopic.topic_name, workspaceId: selectedTopic.workspace_id }
      : null
  )
  ...
  const savedSelectionRef = useRef<{ topicName: string; workspaceId: string } | null>(
    !isFallback && selectedTopic
      ? { topicName: selectedTopic.topic_name, workspaceId: selectedTopic.workspace_id }
      : null
  )
  ...
  useEffect(() => {
    ...
    const loadAndExpand = async () => {
      ...
      try {
        ...
        if (selectedTopic && !hasExpandedRef.current) {
          ...
          const targetWs = allWorkspaces.find(ws => ws.workspaceId === selectedTopic.workspace_id)
          ...
          if (targetWs) {
            ...
            const topics = await loadTopicsForWorkspace(selectedTopic.workspace_id)

            // Find the selected topic
            const targetTopic = topics.find(t => t.name === selectedTopic.topic_name)
            if (targetTopic) {
    ...
    loadAndExpand()
  }, [open, selectedTopic, portalApi, loadWorkspacesForRepo, loadTopicsForWorkspace])
  ...
  useEffect(() => {
    if (!open) {
      ...
      fallbackTopicRef.current = isFallback && selectedTopic
        ? { topicName: selectedTopic.topic_name, workspaceId: selectedTopic.workspace_id }
        : null
      savedSelectionRef.current = !isFallback && selectedTopic
        ? { topicName: selectedTopic.topic_name, workspaceId: selectedTopic.workspace_id }
        : null
    }
  ...
  const handleConfirm = useCallback(() => {
    ...
    onOpenChange(false)
  }, [selected, selectedWorkspaceId, selectedWorkspaceName, onConfirm, onOpenChange])
  ...
  const renderNode = useCallback(
    (node: TreeNode, path: string[], depth: number = 0, parentWorkspaceName?: string) => {
      const isTopic = node.type === "topic"
      const topicData = isTopic ? (node.data as TopicInfo) : null
      const isSelected = isTopic && selected?.name === node.name && selectedWorkspaceId === node.workspaceId
      // Check if this specific item is the auto-detected/default one
      const isFallbackItem = isTopic && fallbackTopicRef.current !== null &&
      ...
      const isSavedItem = isTopic && !isFallbackItem &&
      ...
      return (
        <div key={node.id}>
          ...
          >
            ...
            {isTopic && (
              <ArrowLeftRight className={cn("h-4 w-4", isSelected
                ? selectedColor === "blue"
          ...
          {node.isExpanded && node.children.length > 0 && (
            <div>
              {node.children.map((child) => renderNode(child, currentPath, depth + 1, node.workspaceName || workspaceName))}
            </div>
          )}
        </div>
      )
    },
    [searchQuery, selected, selectedWorkspaceId, selectionIsFallback, toggleNode, handleSelectTopic]
  )
        ...
        {selected && (
          <div className="border rounded-md p-3 bg-muted/30">
            <div className="flex items-center gap-2">
              <ArrowLeftRight className={cn("h-4 w-4", selectionIsFallback
                ? "text-blue-600 dark:text-blue-400"
                : "text-green-600 dark:text-green-400")} />
