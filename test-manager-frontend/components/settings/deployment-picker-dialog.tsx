...
export function DeploymentPickerDialog({
  ...
  const savedSelectionRef = useRef<{
    ...
    !isFallback && selectedDeployment
      ? {
          ...
          workspaceId: selectedDeployment.workspace_id,
        }
      : null,
  );
      ...
      try {
        ...
        if (selectedDeployment && !hasExpandedRef.current) {
          ...
          const targetWs = allWorkspaces.find(
            (ws) => ws.workspaceId === selectedDeployment.workspace_id,
          );
          ...
          if (targetWs) {
            ...
            const deployments = await loadDeploymentsForWorkspace(
              selectedDeployment.workspace_id,
            );

            // Find the selected deployment
            const targetDeployment = deployments.find(
              (d) => d.id === selectedDeployment.deployment_id,
            );
            if (targetDeployment) {
  ...
  useEffect(() => {
    if (!open) {
      ...
      fallbackDeploymentIdRef.current =
        isFallback && selectedDeployment
  ...
  const handleConfirm = useCallback(() => {
    if (selected && selectedWorkspaceId) {
      const reference: DeploymentReference = {
        ...
        internal_url: selected.service_name
          ? `http://${selected.service_name}`
          : selected.publicUrl,
      };
      onConfirm(reference);
    }
  ...
  const renderNode = useCallback(
    (node: TreeNode, path: string[], depth: number = 0) => {
      ...
      const deploymentData = isDeployment
        ? (node.data as DeploymentInfo)
        : null;
      ...
      const isFallbackItem =
        isDeployment && fallbackDeploymentIdRef.current === node.id;
