...
export default function EditEnvironmentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to update environment.",
        variant: "destructive",
      })
    } finally {
  ...
  if (loading || !environment) {
    return (
      <MainLayout backLink={{ href: `/environments/${environmentId}`, label: "Back to Environment" }}>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Status</Label>
                <Select value={formStatus} onValueChange={(v) => setStatus(v as EnvironmentStatus)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button type="button" variant="outline" onClick={() => router.push(`/environments/${environmentId}`)}>
                  Cancel
                </Button>
              </div>
            </form>
